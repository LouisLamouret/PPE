        ...
        <nav class="navbar navbar-default">
            <div class="container-fluid">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="suggestions.php">Les suggestions des utilisateurs</a>
                </div>
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header">
                                <h4 class="title">Récapitulatif des suggestions</h4>
                            </div>
                            <div class="content table-responsive table-full-width">
                               ...
                               <table class="table table-striped">
                            	<thead>
                                	...
                                	<tbody>
                                <?php
                            	$resultat1=$mysqli->query("SELECT suggestions.id_suggestion, suggestions.description_suggestion, suggestions.interlocuteur_id_interlocuteur, interlocuteur.nom_interlocuteur, interlocuteur.prenom_interlocuteur
								FROM suggestions, interlocuteur
								WHERE suggestions.interlocuteur_id_interlocuteur = interlocuteur.id_interlocuteur");
								while ($ligne1 =$resultat1->fetch_assoc()) {
								echo '<tr><td>' .$ligne1['id_suggestion']."</td><td>".$ligne1['description_suggestion']."</td><td>".$ligne1['nom_interlocuteur']." ".$ligne1['prenom_interlocuteur'].'</td><td class="td-actions text-right">
								<a href="?do='.$ligne1['id_suggestion'].'" rel="tooltip" class="btn btn-simple btn-danger btn-icon" data-original-title="Supprimer"><i class="ti-close"></i></a></td></tr>';
								}
								$row = mysqli_num_rows($resultat1);
								if($row < 1) {
								echo '<td><td><td><h3>Il n\'y a pas de suggestions.</h3></td></td></td>';
								}
								
								
								$do = ($_GET['do']);
								if($do != "") {								
								$requete = ("DELETE FROM suggestions WHERE id_suggestion = '".$do."'");
								$resultat = $mysqli->query($requete);
								$row = mysqli_num_rows($resultat);
 								if($row < 1) {
 								echo '<script>alert("Suppression effectuée avec succès.");</script>';
 								echo '<script LANGUAGE="JavaScript">document.location.href="./suggestions.php"</script>';
